...
struct HttpClockApiUtility {
    ...
    static func sendPullCommand(clockAddress: String, onSuccess: @escaping (String) -> Void, onError: @escaping (String) -> Void){
        let payload = try? JSONEncoder().encode(["pull": "yes"])
        if(payload == nil){
            onError("Cannot encode the json payload!")
            return
        }
        let urlString = "http://\(clockAddress)/pull"
        let urlO = URL(string: urlString)
        if let url = urlO {
            var urlRequest = URLRequest(url: url)
            urlRequest.addValue("application/json", forHTTPHeaderField: "Content-Type")
